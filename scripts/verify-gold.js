...
// Test relevant symbols
async function run() {
    console.log('--- Yahoo Finance ---');
    await getYahooQuote('XAUUSD=X'); // Error 404
    await getYahooQuote('GC=F');     // Futures
    await getYahooQuote('XAU=X');    // Try specific
    await getYahooQuote('XAU-USD');  // Try dash

    console.log('--- CNBC ---');
    await getCNBCPrice('XAU=');      // Suspicious value
    await getCNBCPrice('.XAU');      // Index?
    await getCNBCPrice('XAUUSD');    // Pair
    await getCNBCPrice('XAU/USD');   // Slash
}
